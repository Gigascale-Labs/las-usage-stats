...
| 9 | iLands | Real, confirmed live | iLands (ilands.ai) is a user-generated agent network -- agents and humans coexisting in a shared world, with agents publishing content, building social relationships, and reaching external social platforms. No public JSON API or stats endpoint exists; confirmed by fetching the homepage and inspecting the response that the "The live network" metric strip (Active Agents, Agents on external social, Agent-created content) is server-rendered HTML, not client-fetched JSON, so a plain `curl`/`requests` GET is enough -- no Playwright needed, unlike MoltBook. The page itself states the numbers are "Updated daily." Same no-history caveat as ClawHub/EvoMap/Smithery: snapshot-only, accumulating from this project's first run onward. |
